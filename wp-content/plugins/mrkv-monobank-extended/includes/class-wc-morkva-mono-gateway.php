...
class WC_Gateway_Morkva_Mono extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        add_action('woocommerce_thankyou', array( $this, 'return_handler' ) );
    ...
    public function init_form_fields() 
    {
        ...
        $this->form_fields = array(
            ...
            'mono_delivery_methods_np_brnm' => array(
                'title' => __( 'Nova Poshta', 'morkva-monobank-extended' ) . '<b><a href="https://morkva.co.ua/shop/monobank-extended-pro-lifetime/" target="blank">' . __( 'Only in Pro version', 'morkva-monobank-extended' ) . '</a></b>',
                'type' => 'select',
            ...
            'display_mode_minicart' => array(
                'title' => '',
                'type' => 'checkbox',
                'label' => '<span>' . __( 'Minicart', 'morkva-monobank-extended' )  . '</span>',
                'default' => 'yes'
            ),
            'display_mode_cartmain' => array(
                'title' => '',
                'type' => 'checkbox',
                'label' => '<span>' . __( 'Cart', 'morkva-monobank-extended' )  . '</span>',
                'default' => 'yes'
            ),
            'display_mode_checkout' => array(
                'title' => '',
                'type' => 'checkbox',
                'label' => '<span>' . __( 'Checkout', 'morkva-monobank-extended' )  . '</span>',
                'default' => 'yes'
            ),
    ...
    {   
		...
		// Force update Order information in a case if made payment
		// but didn't receive any information from Payment gateway
		if ( !$mrkv_mono_callback ) {
			global $woocommerce;

			$paid_order_id = null;
			// Check if the REDIRECT_SCRIPT_URL element exists in the $_SERVER array
			if (isset($_SERVER['REDIRECT_SCRIPT_URL'])) {
				// We get the value of REDIRECT_SCRIPT_URL
				$redirect_url = filter_var($_SERVER['REDIRECT_SCRIPT_URL'], FILTER_SANITIZE_URL);

				// Define a regular expression to search for order ID
				$pattern = '/order-received\/(\d+)\//';

				// Trying to find the order ID in the line
				if (preg_match($pattern, $redirect_url, $matches)) {
					// Extracting the order ID
					$order_id = $matches[1]; // Index 1 contains the captured value (order ID)
					$paid_order_id = htmlspecialchars($order_id, ENT_QUOTES, 'UTF-8');
				}
			}
